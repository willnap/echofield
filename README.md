...
Each source sits at world coordinates and passes through HRTF independently, so three birds in different trees produce three distinct spatial positions. This is impossible with one ambient track. 82 OGG samples across 12 subcategories play with per-instance pitch (±5%) and gain (±2 dB) variation.
...
where *x* is membrane displacement, ω₀ the natural frequency (2.5–5.5 kHz for song sparrow), β₁ < 0 injects energy (the Hopf instability), β₂ and β₃ saturate amplitude, *p* is driving air pressure, and κ the nonlinear restoring coefficient. Below threshold pressure the membrane sits still; above it, oscillation starts abruptly, which matches how real calls begin without fade-in [3]. Integrated at 44.1 kHz via RK4 (Euler diverges at realistic parameter values). Eight species, tracheal resonance via FIR comb filter, stochastic variation in pitch (12%), tempo (25%), pressure (25%).
...
Almost everything here connects back to module content. I encountered Eyring RT60, spectral decay, ITD and ILD, precedence, critical distance, and air absorption in lectures and then had to make them work in real time on live geometry. That gap between understanding the equation and getting it to sound right is where most of the calibration went. Perceived loudness scaling looks clean on a slide; getting it to feel correct for a bird 40 blocks away in a reverberant cave took weeks.
